...
def process_data(data_list):
    ...
    for i in data_list:
        date_id = i[0].strip('\"')
        for j in i[1]:
            product_id = int(float(j[1].strip('\"').lstrip('0')))
            cust_id = j[0].strip('\"')
            date_cust_id = date_id + '-' + cust_id
            all_list.append([date_cust_id, product_id])
...
if __name__ == '__main__':
    ...
    data = csv_data.map(lambda x: (x.split(',')[0], (x.split(',')[1], x.split(',')[5]))) \
    ...
    business_data = csv_data.map(lambda x: (x.split(',')[0], x.split(',')[1])) \
        ...
        .filter(lambda x: len(x) > int(filter_threshold))
